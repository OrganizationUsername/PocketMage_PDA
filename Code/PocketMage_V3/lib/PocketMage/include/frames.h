...
template <size_t MAX_LINES, size_t BUF_BYTES> struct FixedArenaSource : TextSource {
  char buf[BUF_BYTES];
  ...
  bool pushLine(const char* s, uint16_t L, uint8_t flags = LF_NONE) {
    if (nLines >= MAX_LINES || used + L + 1 > BUF_BYTES)
      return false;
    memcpy(buf + used, s, L);
    buf[used + L] = '\0';
...
class Frame {
 public:
  ...
  const GFXfont* font = (GFXfont*)&FreeSerif9pt8b;
  ...
  Frame(int left, int right, int top, int bottom, const TextSource* linesPtr, bool cur = false,
        bool bx = false)
  ...
  Frame(int left, int right, int top, int bottom, const uint8_t* bitmapPtr, int width, int height,
  ...
  bool hasText() const {
    return kind == Kind::text && source;
  }
  bool hasBitmap() const {
    return kind == Kind::bitmap && bitmap;
  }
};
...
// Forward declaration
extern const GFXfont FreeSerif9pt8b;
