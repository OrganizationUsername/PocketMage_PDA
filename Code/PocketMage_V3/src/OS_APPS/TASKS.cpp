      ...
      if (newState) {
        ...
        if (!tasks.empty()) {
          ...
          for (int i = 0; i < loopCount; i++) {
            ...
            // Serial.print(tasks[i][0].c_str());
            // Serial.println(convertDateFormat(tasks[i][1]).c_str());
            ESP_LOGI("TASKS", "%s, %s", tasks[i][0].c_str(),
                     convertDateFormat(tasks[i][1]).c_str());  // TODO: Come up with some tag
          }
        } else
          EINK().drawStatusBar("No Tasks! Add New Task (N)");
